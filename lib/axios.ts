 ...
 * Callers that intentionally branch on a specific failure status (e.g. a 404
 * "not found" empty state) can read `.status` off the caught `ApiError`
 * instead of inspecting a returned body.
 */
 ...
 *
 * Exported (not just used internally by the helpers below) so callers that
 * bypass the axiosGet/Post/Put/Patch/Delete wrappers — e.g. to call a route
 * that predates this convention and returns real HTTP status codes instead
 * of an embedded `status` field — can still normalize into the same
 * `ApiError` shape. See lib/api/schedule.ts. */
 ...
 *
 * Some routes (e.g. GET /api/profile for a brand-new user) still embed real
 * `data` alongside a non-2xx `status` — fields that are always known
 * regardless of whether the rest of the resource has been filled in yet
 * shouldn't get thrown away just because the overall status is "empty".
 * `error.data` (set by toApiError below) is the original parsed response
 * body, so recover it when the route provided one instead of always
 * reconstructing a data-less shell.
 */
export function allowStatuses<T>(
  ...
  if (error instanceof ApiError && statuses.includes(error.status)) {
    const body = error.data as IResponse<T> | undefined
...
// Not in the reference convention this file was adopted from — added
// because /api/schedule/doses/[id] (an already-existing route, kept as-is
// per lib/api/schedule.ts's comments) uses PATCH rather than PUT.
